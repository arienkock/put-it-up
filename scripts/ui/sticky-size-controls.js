/**
 * Creates and manages the sticky sizing control UI
 * Allows users to grow or shrink the selected sticky in different directions
 * 
 ...
 * @param {string|number} stickyId - The selected sticky id to resize
 * @returns {HTMLElement|null} The sizing controls element, or null if already exists
 */
export function createStickySizeControls(board, root, activatingEvent, stickyId) {
  ...
  sizingControls.addEventListener("click", (event) => {
    ...
    if (side) {
      board.changeStickySize(stickyId, isGrow, side);
    }
  });
